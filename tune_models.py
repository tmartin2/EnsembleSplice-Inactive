...
def tune_cnn2(
    ...
    tuner_pooling_2 = tuner.Int(name='pooling 2', min_value=1, max_value=10, step=1)
    model.add(tf.keras.layers.MaxPooling1D(pool_size=tuner_pooling_2, padding="same"))
...
def tune_dnn1(
    ...
    tuner_regularizer_3= tuner.Choice(name='regularizer 3', values=[0.001, 0.0025, 0.005, 0.01, 0.025, 0.05])
    model.add(tf.keras.layers.Dense(
        units=tuner_units_3,
        activation='relu',
...
def run(**kwargs):
    ...
    for ss_type in kwargs['splice_sites']:
        ...
        tuner01.search(conv2d_X_trn, y_trn, epochs=200, validation_split=0.2, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, min_delta=0.001)])
        ...
        tuner02.search(X_trn, y_trn, epochs=200, validation_split=0.2, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, min_delta=0.001)])
        ...
        tuner03.search(X_trn, y_trn, epochs=200, validation_split=0.2, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, min_delta=0.001)])
        ...
        tuner05.search(X_trn, y_trn, epochs=200, validation_split=0.2, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, min_delta=0.001)])
        ...
        tuner09.search(X_trn, y_trn, epochs=200, validation_split=0.2, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, min_delta=0.001)])
    ...
    # cnn1
        print(f"""
    The hyperparameter search is complete for CNN01.
    ...
    max pool 1: ({tuner01_best_hps.get('pooling 1')}, {tuner01_best_hps.get('pooling 1')})
    3rd layer filters: {tuner01_best_hps.get('filters 3')}
    3rd layer kernal: ({tuner01_best_hps.get('kernal 3')}
    max pool 2: ({tuner01_best_hps.get('pooling 2')}, {tuner01_best_hps.get('pooling 2')})
    ...
    # cnn2
        print(f"""
    The hyperparameter search is complete for CNN02.
    ...
    # cnn3
        print(f"""
    The hyperparameter search is complete for CNN03.
    ...
    # dnn1
        print(f"""
    The hyperparameter search is complete for DNN01.
    ...
    # dnn2
        print(f"""
    The hyperparameter search is complete for DNN02.
    ...
    # dnn3
        print(f"""
    The hyperparameter search is complete for DNN03.
